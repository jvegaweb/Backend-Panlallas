...
<div class="card">
    <div class="card-header">
        {{ trans('global.edit') }} {{ trans('cruds.espacioscomune.title_singular') }}
    </div>

    <div class="card-body">
        <form method="POST" action="{{ route("admin.espacioscomunes.update", [$espacioscomune->id]) }}" enctype="multipart/form-data">
            @method('PUT')
            @csrf
            <div class="form-group">
                <label for="name">{{ trans('cruds.espacioscomune.fields.name') }}</label>
                <input class="form-control {{ $errors->has('name') ? 'is-invalid' : '' }}" type="text" name="name" id="name" value="{{ old('name', $espacioscomune->name) }}">
                @if($errors->has('name'))
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('id_pantalla') ? 'is-invalid' : '' }}" name="id_pantalla_id" id="id_pantalla_id">
                    @foreach($id_pantallas as $id => $entry)
                        <option value="{{ $id }}" {{ (old('id_pantalla_id') ? old('id_pantalla_id') : $espacioscomune->id_pantalla->id ?? '') == $id ? 'selected' : '' }}>{{ $entry }}</option>
                    @endforeach
                </select>
