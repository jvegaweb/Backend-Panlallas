    ...
    <div class="card-body">
        <form method="POST" action="{{ route("admin.clientes.store") }}" enctype="multipart/form-data">
            ...
            <div class="form-group">
                ...
                <input class="form-control {{ $errors->has('rut') ? 'is-invalid' : '' }}" type="number" name="rut" id="rut" value="{{ old('rut', '') }}" step="1" required>
                @if($errors->has('rut'))
                    <div class="invalid-feedback">
                        {{ $errors->first('rut') }}
                    </div>
            ...
            <div class="form-group">
                ...
                <select class="form-control select2 {{ $errors->has('screens') ? 'is-invalid' : '' }}" name="screens[]" id="screens" multiple>
                    @foreach($screens as $id => $screen)
                        <option value="{{ $id }}" {{ in_array($id, old('screens', [])) ? 'selected' : '' }}>{{ $screen }}</option>
                    @endforeach
                </select>
